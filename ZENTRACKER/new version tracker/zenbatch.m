...
%added: darkfield option
%fixed: when detecting light but not objects, the previously detected objects are no longer cleared
...
function zenbatch
    ...
    handles.dotrack = uicontrol(handles.datapanel, 'Style', 'Checkbox', 'String', 'Detect and track objects', 'Value', 1, 'Position',[0.00 0.10 0.12 0.05]);
    handles.dolight = uicontrol(handles.datapanel, 'Style', 'Checkbox', 'String', 'Detect light', 'Value', 0, 'Position',[0.13 0.10 0.12 0.05]);
    handles.dovalidity = uicontrol(handles.datapanel, 'Style', 'Checkbox', 'String', 'Check validity', 'Value', 1, 'Position',[0.00 0.00 0.12 0.05]);
    handles.doduration = uicontrol(handles.datapanel, 'Style', 'Checkbox', 'String', 'Check duration', 'Value', 0, 'Position',[0.13 0.00 0.12 0.05]);
    ...
    function analyse(hobj, eventdata) %#ok<INUSD>
        ...
        for currentmovie = 1:numel(selectedfiles)
            ...
            try
                ...
                if isfield(ztdata, 'darkfield')
                    darkfield = ztdata.darkfield;
                else
                    darkfield = false;
                end
                ...
                objects = [];
                %only bother with opening the movie if we need to
                if get(handles.dotrack, 'Value') || get(handles.dolight, 'Value')
                ...
                if ~get(handles.dotrack, 'Value') %if not tracking, we need to get the objects structure from the savefile
                ...
                if get(handles.dolight, 'Value') %detect light only if asked to
                    %detecting a sudden increase in brightness
                    previousaverage = 0;
                    for i=1:lastframe
                        ...
                        if i>1 && currentaverage/previousaverage > 1.5
                            flashindices = i;
                            break;
                        end
                ...
                if get(handles.dolight, 'Value') && exist('flashindices', 'var') == 1
                    save(selectedfiles{currentmovie}, 'objects', 'meanperimeter', 'flashindices', '-append');
                else
