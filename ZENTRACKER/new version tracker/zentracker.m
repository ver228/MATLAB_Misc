%DONE:
%added: button for spectral density analysis of orientation-changes (required min valid stretch of 20 frames)
%added: ability to plot orientation
%added: orientation measurement
%added: darkfield checkbox to invert the image
%changed: reversal proportion now disregards unknown behaviours instead of treating them as non-reversals
%improved: recheck validity now respects from and until time interval
%added: ability to detect sudden increases in overall brightness
%fixed: completely uniform RGB images are no longer autoscaled (which would produce an error)
%fixed: "only longest n" now sorts the objects according to the time they exist within the time period specified (rather than over the entire movie)
%improved: no longer even attempts thresholding when the thresholding area is not yet set up
%improved: faster minimum duration object deletion 
%changed: minimum detection size default changed from 15000 um^2 to 20000 um^2
%TODO:
%cleanup: it should be clear which variables denote time in frames and which in seconds
%fix: avoid the case where we analyse a movie, select a different movie, but before loading the new movie, we save the old movie: this would store the wrong selectedmovies
%add: ability to restrict when tracking should occur with "Track objects". Perhaps using the "from (s)" and "until (s)" fields.
%improve: "consistent" manual reversal detection should be a bit cleaner, e.g. it should display the whole consistent interval instead of only the samedirection; samedirection > revdurationmax check should come before potential asking for dwelling headdirection userinput; if userinput indicates that headdirection is inconsistent with centroid movement direction, there should be a smarter fallback to inconsistent headdirection checking (in particular, as it is now, we're discarding the samedirection userinput)
%improve: incorporate fuzzy validity checking as an advanced option in validity checking (for multiple parameters?)
...
%    thresholdingfilter = daubcqf(4); %fspecial('gaussian',[15,15],2);
    ...
    handles.believefps = uicontrol(handles.datapanel,'Style','Checkbox','String','Assume movie FPS is correct', 'Position',[0.60 0.0833 0.40 0.0333],'Value', believefps, 'Callback', {@setvalue, 'logical', 'setglobal', 'believefps'});
    handles.cachemovie = uicontrol(handles.datapanel,'Style','Checkbox','String','Cache movie', 'Position',[0.60 0.05 0.40 0.0333],'Value',believefps, 'Value', cachemovie, 'Callback', {@setvalue, 'logical', 'setglobal', 'cachemovie'});
    ...
    handles.identitydisplay = uicontrol(handles.displaysettingspanel , 'Style', 'Checkbox', 'String', 'Identities', 'Position', [0.05 0.75 0.90 0.25], 'Value', identitydisplay, 'Callback', {@setvalue, 'logical', 'setglobal', 'identitydisplay', 'showit'});
    handles.detectionareadisplay = uicontrol(handles.displaysettingspanel , 'Style', 'Checkbox', 'String', 'Detection area', 'Position', [0.05 0.50 0.90 0.25], 'Value', detectionareadisplay, 'Callback', {@setvalue, 'logical', 'setglobal', 'detectionareadisplay', 'showit'});
    handles.thresholdeddisplay = uicontrol(handles.displaysettingspanel , 'Style', 'Checkbox', 'String', 'Thresholded pixels', 'Position', [0.05 0.25 0.90 0.25], 'Value', thresholdeddisplay, 'Callback', {@setvalue, 'logical', 'setglobal', 'thresholdeddisplay', 'showit'});
    handles.measurementareadisplay = uicontrol(handles.displaysettingspanel , 'Style', 'Checkbox', 'String', 'Measurement area', 'Position', [0.05 0.00 0.90 0.25], 'Value', measurementareadisplay, 'Callback', {@setvalue, 'logical', 'setglobal', 'measurementareadisplay', 'showit'});
    ...
    handles.measurementwhat = uicontrol(handles.validpanel, 'Style','Popupmenu', 'String', {'Add to measurement area', 'Remove from measurement area'}, 'Position', [0.05 0.90 0.90 0.10], 'Value', 1);
    ...
    handles.setmeasurementarea = uicontrol(handles.validpanel, 'Style', 'Pushbutton', 'String', 'Mark area', 'Position', [0.50 0.63 0.50 0.17], 'Callback', {@markarea, 'valid'});

    handles.validadvanced = uicontrol(handles.validpanel, 'Style', 'Pushbutton', 'String', 'Advanced settings', 'Position', [0.10 0.45 0.80 0.10], 'Callback', @validityadvanced);
    handles.validcheck = uicontrol(handles.validpanel, 'Style', 'Pushbutton', 'String', '(Re)check validity!', 'Position', [0.05 0.25 0.90 0.15], 'Callback', @validitycheck);
    ...
    handles.plotwhat = uicontrol(handles.resultspanel, 'Style', 'Popupmenu', 'String', {'Speed', 'X coordinates', 'Y coordinates', 'Length', 'Width', 'Area', 'Perimeter', 'Eccentricity', 'Solidity', 'Orientation', 'Compactness', 'Direction change', 'Reversals', 'Omega turns', 'Valid N-numbers'}, 'Position', [0.05 0.15 0.90 0.10], 'Value', 1);
    ...
    trytoload = {'saveversion', 'objects', 'timefrom', 'timeuntil', 'scalingfactor', 'movingaverage', 'speedsmoothing', 'meanperimeter',...
            'longest', 'validdurationminimum', 'validspeedmin', 'validspeedmax', 'validlengthmin', 'validlengthmax', 'measurementarea',...
            ...
            'identitydisplay', 'detectionareadisplay', 'thresholdeddisplay', 'measurementareadisplay', 'wormid', 'allobjects', 'averagedisplay', 'detectionarea',... 
    ...
    trytosetvalue = {'identitydisplay', 'detectionareadisplay', 'thresholdeddisplay', 'measurementareadisplay', 'wormshowframeslider', 'allobjects', ...
        ...
        if ~isempty(objects)
            ...
            if averagedisplay || forceaverage %mean
                ...
                if overallcountdata > 0
                    ...
                    byframemeandata = mean(meandata(~isnan(meandata)));
                else
                    overallmeandata = NaN;
                    byframemeandata = NaN;
                end
                ...
                if movingaverage > 1/framerate
                    meandata = movingaveragefilterwithoutnan(meandata, round(movingaverage*framerate));
                end
    ...
    function readmovie(hobj, eventdata) %#ok<INUSD>
        ...
        for i=1:numel(selectedfiles)
            ...
            if ~(ispc && strcmpi(currentfullfile(end-3:end), '.mov')) %reading of quicktime movies is not supported on Windows, and in my experience can sometimes cause the wrong frames to be displayed (when not all frames are cached, which we cannot ensure), so we'll fall back to other options, probably mmread
                ...
                if ~successfullyread
                    try
                        ...
                        moviereaderobjects(end+1).reader = mmreader(currentfullfile); %#ok<AGROW,TNMLP>
            ...
            if believefps && ~isempty(currentframerate) && ~isnan(currentframerate) %if we want to believe the framerate, and the framerate value exists
            ...
            if i==1
                ...
                elseif ~isempty(findstr(class(firstframe), 'int'))
                ...
                detectionarea = false(movieheight, moviewidth); %logical matrix representing which pixels can be thresholded
                measurementarea = false(movieheight, moviewidth); %logical matrix representing centroids at which pixels can be considered valid
        ...
        if sum(isnan(nf(:))) > 0 && ~cancelling %if there are movies with still unknown number of frames (and not because the user cancelled), we'll tell the user that we're checking the number of frames
        ...
        %verifying the number of frames that are in the movie (bah, but necessary for the stupid quicktime mov files where neither mmreader nor mmread doesn't know the actual number for sure!)
        if ~cancelling
            for i=1:numel(selectedfiles)
                ...
                frameindicator(end+1:end+nf(i)) = 1:nf(i);
    ...
    function smoothedspeeds = smoothspeed(objectindex, frameindices)
        ...
        for i=1:numel(frameindices) %going through the indices, which makes i the index of the index (necessary for the general case because we might get nonconsecutive frameindices as the argument)
            ...
            if lookfromindex >= 1 && lookuntilindex <= objects(objectindex).duration %bounds check
                ...
                %{
                if ~allvalid && ~invalid(objectindex, lookfromindex) && ~invalid(objectindex, lookuntilindex)
                    fprintf('Object %d is subtly invalid between frames %d and %d\n', objectindex, objects(objectindex).frame(lookfromindex), objects(objectindex).frame(lookuntilindex));
                end
                %}
                if allvalid
                    smoothedspeeds(i) = realsqrt((objects(objectindex).x(lookuntilindex)-objects(objectindex).x(lookfromindex))^2 + (objects(objectindex).y(lookuntilindex)-objects(objectindex).y(lookfromindex))^2)/(speedsmoothing/framerate);
                end
            end
        end
    ...
    function updatefilelist(hobj,eventdata) %#ok<INUSD>
        ...
        moviefileextensions = {'mpg', 'avi', 'wmv', 'asf', 'mpeg', 'mp4', 'mov', 'm4v', 'tif', 'tiff'};
    ...
    function isitinvalid = invalid (objectindex, frameindex, parameter) %validity checking the specified frame, and (if exists) the frame before. If neither frames are invalid, the checked frame is deemed acceptable
        
        if exist('parameter', 'var') ~= 1
    ...
    function wormshow (wheretoshow, enforcesimpledisplay, frametoshow)
        ...
        if ~enforcesimpledisplay
            ...
            if thresholdeddisplay || detectionareadisplay || measurementareadisplay
...
%                    thresholdedimage = bwlabel(thresholdedimage);
    ...
    function markarea(hobj, eventdata, whattomark) %#ok<INUSL>
        ...
        elseif strcmpi(whattomark, 'valid')
            ...
            markas = get(handles.measurementwhat, 'Value') == 1;
    ...
    function debuggingfunction (hobj, eventdata) %#ok<INUSD>
        ...
        intervals1speed = [];
        intervals2speed = [];
        intervals1rev = cell(0);
        intervals2rev = cell(0);
        for i=1:numel(objects)
            if any(objects(i).frame >= interval1first & objects(i).frame <= interval1last) && any(objects(i).frame >= interval2first & objects(i).frame <= interval2last)
                if any(objects(i).behaviour(find(objects(i).frame == interval1last)+1:find(objects(i).frame == interval2first)-1) == CONST_BEHAVIOUR_INVALID)
                    continue
                end
                workable1first = NaN;
                workable1last = find(objects(i).frame == interval1last);
                workable2first = find(objects(i).frame == interval2first);
                workable2last = NaN;
                for j=interval1last:-1:interval1first
                    currentframe = find(objects(i).frame == j);
                    if isempty(currentframe) || objects(i).behaviour(currentframe) == CONST_BEHAVIOUR_INVALID
                        break
                    else
                        workable1first = currentframe;
                    end
                end
                for j=interval2first:+1:interval2last
                    currentframe = find(objects(i).frame == j);
                    if isempty(currentframe) || objects(i).behaviour(currentframe) == CONST_BEHAVIOUR_INVALID
                        break
                    else
                ...
                if ~isnan(workable1first) && ~isnan(workable2last) && objects(i).frame(workable1last)-objects(i).frame(workable1first)+1 >= intervalmin && objects(i).frame(workable2last)-objects(i).frame(workable2first)+1 >= intervalmin
                    fprintf('%d to %d\n', objects(i).frame(workable1first), objects(i).frame(workable2last));
                    intervals1speed(end+1) = nanmean(objects(i).speed(workable1first:workable1last));
                    intervals2speed(end+1) = nanmean(objects(i).speed(workable2first:workable2last));
                    intervals1rev{end+1} = objects(i).behaviour(workable1first:workable1last) == CONST_BEHAVIOUR_REVERSAL | objects(i).behaviour(workable1first:workable1last) == CONST_BEHAVIOUR_OMEGA;
                    intervals2rev{end+1} = objects(i).behaviour(workable2first:workable2last) == CONST_BEHAVIOUR_REVERSAL | objects(i).behaviour(workable2first:workable2last) == CONST_BEHAVIOUR_OMEGA;
                end
            end
        end
        
        save('currentlytryingnpr1d', 'intervals1speed', 'intervals2speed', 'intervals1rev', 'intervals2rev');
        %}
        
        %{
        for i=1:numel(objects)
            objects(i).behaviour(objects(i).behaviour == CONST_BEHAVIOUR_UNKNOWN) = CONST_BEHAVIOUR_INVALID;
        end
        %}
        
        %currentframe = readframe(wormshowframe);
        %{
        toaverage = NaN(movieheight, moviewidth, 20);
        whichindex = 0;
        for i=0:19
        ...
        averaged = median(toaverage, 3);
        
        if any(detectionarea(:))
            pixelstouse = averaged(detectionarea);
        end

        adjusted = averaged-mean(pixelstouse);
        adjusted = adjusted/std(pixelstouse)*(1/20);
        adjusted = adjusted+0.5;

        adjusted(adjusted < 0.0) = 0.0;
        adjusted(adjusted > 1.0) = 1.0;

        adjusted = adjusted * pixelmax;
        %}
        
        %{
        for i=1:numel(objects)
            for j=1:objects(i).duration
                if objects(i).behaviour(j) == CONST_BEHAVIOUR_OMEGA && objects(i).compactness(j) > 30 && objects(i).eccentricity(j) < 0.8
                    fprintf('%d at %f\n', i, objects(i).time(j));
                end
            end
        end
        ...
        %{
        wormtodraw = 611;
        drawfrom = converttoframe(523);
        drawuntil = converttoframe(528.5);
        figure;
        for i=drawfrom:drawuntil
            subplot(1, 12, i-drawfrom+1);
            currentframe = readframe(i);
            croppednear = currentframe(round(objects(wormtodraw).y(objects(wormtodraw).frame == i))-24:round(objects(wormtodraw).y(objects(wormtodraw).frame == i))+24, round(objects(wormtodraw).x(objects(wormtodraw).frame == i))-24:round(objects(wormtodraw).x(objects(wormtodraw).frame == i))+24);
            imshow(croppednear, []);
            title(sprintf('%.1f s', converttotime(i)));
        end
        %}
        ...
        for i=1:numel(objects)
            for j=1:objects(i).duration
                if objects(i).time(j) > 120
                    continue
                end
                if objects(i).behaviour(j) == CONST_BEHAVIOUR_INVALID && measurementarea(round(objects(i).y(j)), round(objects(i).x(j)))
                    length(end+1) = objects(i).length(j)*scalingfactor;
                    area(end+1) = objects(i).area(j)*scalingfactor^2;
                    width(end+1) = objects(i).width(j)*scalingfactor;
                    perimeter(end+1) = objects(i).perimeter(j)*scalingfactor;
                    eccentricity(end+1) = objects(i).eccentricity(j);
                    solidity(end+1) = objects(i).solidity(j);
                    compactness(end+1) = objects(i).compactness(j);
                end
                if objects(i).behaviour(j) ~= CONST_BEHAVIOUR_INVALID
                    vlength(end+1) = objects(i).length(j)*scalingfactor;
                    varea(end+1) = objects(i).area(j)*scalingfactor^2;
                    vwidth(end+1) = objects(i).width(j)*scalingfactor;
                    vperimeter(end+1) = objects(i).perimeter(j)*scalingfactor;
                    veccentricity(end+1) = objects(i).eccentricity(j);
                    vsolidity(end+1) = objects(i).solidity(j);
                    vcompactness(end+1) = objects(i).compactness(j);
                end
            end
        end
        %}
        ...
        %{
        for i=1:numel(objects)
            objects(i).behaviour(objects(i).behaviour == CONST_BEHAVIOUR_UNKNOWN) = CONST_BEHAVIOUR_FORWARDS;
        end
        %}
        ...
        for i=1:numel(objects)
            for j=2:objects(i).duration
                if (objects(i).behaviour(j-1) == CONST_BEHAVIOUR_INVALID && objects(i).behaviour(j) == CONST_BEHAVIOUR_FORWARDS) || (j == 2 && all(objects(i).behaviour(1:2) == CONST_BEHAVIOUR_FORWARDS))
                    for k=j+1:objects(i).duration
                        if objects(i).behaviour(k) == CONST_BEHAVIOUR_INVALID || (k==objects(i).duration == objects(i).behaviour(k) == CONST_BEHAVIOUR_FORWARDS)
                            if k-j >= 15
                                %disp('fine');
                            else
                                if (j == 2 && all(objects(i).behaviour(1:2) == CONST_BEHAVIOUR_FORWARDS))
                                    objects(i).behaviour(1) = CONST_BEHAVIOUR_UNKNOWN;
                                    howmanycleared = howmanycleared + 1;
                                end
                                if (k==objects(i).duration == objects(i).behaviour(k) == CONST_BEHAVIOUR_FORWARDS)
                                    objects(i).behaviour(k) = CONST_BEHAVIOUR_UNKNOWN;
                                    howmanycleared = howmanycleared + 1;
                                end
                                objects(i).behaviour(j:k-1) = CONST_BEHAVIOUR_UNKNOWN;
                                howmanycleared = howmanycleared + k-j;
                                break
                            end
                        elseif objects(i).behaviour(k) == CONST_BEHAVIOUR_REVERSAL || objects(i).behaviour(k) == CONST_BEHAVIOUR_OMEGA
                            break
                        end
                    end
                end
        ...
        %thresholdingfilter = daubcqf(2);
        %disklike = strel('disk', 1, 4);
        ...
        %{
        figure; imshow(cachedframe(50:end-50, 50:end-50), []); title('original');
        
        h = daubcqf(4);
        
        %for i=3:1:8
            i = 10;
            denoised = denoise(cachedframe, h, 1, [0 i 0 0 0 0]);
            figure; imshow(denoised(50:end-50, 50:end-50), []); title(['denoised (1) ' num2str(i)]);
        %end
        ...
        %{
        gaussianfilter = fspecial('gaussian',[15,15],2); %Gaussian filter to help find continuous areas
        disklike = strel('disk', 3, 4);
        cachedindex = NaN;
        wormshow;
        %}
        ...
        %{
        a = figure;
        b = axes('Parent',a,'Visible','on','Position',[0.00 0.00 1.00 1.00]);
        wormshow(b);
        %}
        ...
        for i=1:numel(objects)
            whichframes = objects(i).time >= 950 & objects(i).time <= 1050;
            startat = find(whichframes, 1, 'first');
            endat = find(whichframes, 1, 'last');
            for j=startat:endat
                if objects(i).behaviour(j) == CONST_BEHAVIOUR_OMEGA 
                    if objects(i).behaviour(j-1) ~= CONST_BEHAVIOUR_OMEGA
                        whichomega = whichomega + 1;
                    end
                    whichframeinomega = whichframeinomega + 1;
                    
                    if whichframeinomega == 1
                        omegastuff{whichomega}.x(1:11) = objects(i).x(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.y(1:11) = objects(i).y(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.speed(1:11) = objects(i).speed(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.length(1:11) = objects(i).length(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.width(1:11) = objects(i).width(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.area(1:11) = objects(i).area(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.perimeter(1:11) = objects(i).perimeter(j-10:j); %#ok<AGROW>
                        omegastuff{whichomega}.eccentricity(1:11) = objects(i).eccentricity(j-10:j); %#ok<AGROW>
                        whichframeinomega = 12;
                        
                        lastbad = find(objects(i).behaviour(j-10:j-1) == CONST_BEHAVIOUR_OMEGA, 1, 'last');
                        omegastuff{whichomega}.x(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.y(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.speed(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.length(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.width(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.area(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.perimeter(1:lastbad) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.eccentricity(1:lastbad) = NaN; %#ok<AGROW>
                    end
                    
                    omegastuff{whichomega}.x(whichframeinomega) = objects(i).x(j); %#ok<AGROW>
                    omegastuff{whichomega}.y(whichframeinomega) = objects(i).y(j); %#ok<AGROW>
                    omegastuff{whichomega}.speed(whichframeinomega) = objects(i).speed(j); %#ok<AGROW>
                    omegastuff{whichomega}.length(whichframeinomega) = objects(i).length(j); %#ok<AGROW>
                    omegastuff{whichomega}.width(whichframeinomega) = objects(i).width(j); %#ok<AGROW>
                    omegastuff{whichomega}.area(whichframeinomega) = objects(i).area(j); %#ok<AGROW>
                    omegastuff{whichomega}.perimeter(whichframeinomega) = objects(i).perimeter(j); %#ok<AGROW>
                    omegastuff{whichomega}.eccentricity(whichframeinomega) = objects(i).eccentricity(j); %#ok<AGROW>
                    
                else
                    %{
                    if j>1 && objects(i).behaviour(j-1) == CONST_BEHAVIOUR_OMEGA
                        omegastuff{whichomega}.x(whichframeinomega+1:whichframeinomega+10) = objects(i).x(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.y(whichframeinomega+1:whichframeinomega+10) = objects(i).y(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.speed(whichframeinomega+1:whichframeinomega+10) = objects(i).speed(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.length(whichframeinomega+1:whichframeinomega+10) = objects(i).length(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.width(whichframeinomega+1:whichframeinomega+10) = objects(i).width(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.area(whichframeinomega+1:whichframeinomega+10) = objects(i).area(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.perimeter(whichframeinomega+1:whichframeinomega+10) = objects(i).perimeter(j:j+9); %#ok<AGROW>
                        omegastuff{whichomega}.eccentricity(whichframeinomega+1:whichframeinomega+10) = objects(i).eccentricity(j:j+9); %#ok<AGROW>
                        
                        firstbad = find(objects(i).behaviour(j:j+9) == CONST_BEHAVIOUR_OMEGA, 1, 'first');
                        omegastuff{whichomega}.x(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.y(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.speed(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.length(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.width(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.area(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.perimeter(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                        omegastuff{whichomega}.eccentricity(whichframeinomega+firstbad:whichframeinomega+10) = NaN; %#ok<AGROW>
                    end
                    %}
                    whichframeinomega = 0;
                end
        ...
        fprintf('drift speed over the omega:\n');
        for i=1:numel(omegastuff)
            fprintf('%f\n', hypot(omegastuff{i}.x(end)-omegastuff{i}.x(11), omegastuff{i}.y(end)-omegastuff{i}.y(11)) * scalingfactor / (numel(omegastuff{i}.x)-11))
        end
        
        fprintf('\ndrift speed before the omega:\n');
        for i=1:numel(omegastuff)
            fprintf('%f\n', hypot(omegastuff{i}.x(10)-omegastuff{i}.x(1), omegastuff{i}.y(10)-omegastuff{i}.y(1)) * scalingfactor / 9)
        end
        ...
        rr = 0;
        for i=1:numel(objects)
            f = sum(objects(i).behaviour == CONST_BEHAVIOUR_FORWARDS);
            r = sum(objects(i).behaviour == CONST_BEHAVIOUR_REVERSAL);
            ff = ff + f;
            rr = rr + r;
            fprintf('%d: %.1f\n', i, f/(f+r)*100);
        end
        fprintf('overall: %.1f forwards\n', ff/(ff+rr)*100);
        %}
        ...
        %whos('moviecache(1).data');
        
        %{
        currentframe = readframe(11, false);
        
        pixelstouse = currentframe(detectionarea);
        
        [b, T] = histeq(pixelstouse, pixelmax);
        
        %newframe = currentframe ./ max(currentframe(:));
        newframe = double(currentframe);
        
        for i=1:numel(T)
            newframe(currentframe>=i-1 & currentframe<i) = T(i);
        end
        
        newframe = newframe * pixelmax;
        %}
        ...
        for i=1:lastframe
            currentframe = readframe(i);
            currentframe = currentframe(detectionarea);
            normalizedmean(i) = mean(currentframe);
            normalizedstd(i) = std(currentframe);
        end
        
        figure; plot(normalizedmean); title('mean');
        figure; plot(normalizedstd); title('std');
        %}
        
        %{
        for i=1:2:21
            %filtereddata = medfilt2(averageintensity, [i, i]);
            gf = fspecial('gaussian', [i*4+1, i*4+1], i);
            filtereddata = imfilter(averageintensity, gf);
            subplot(1, 2, 1); imshow(averageintensity, []); title('original');
            subplot(1, 2, 2); imshow(filtereddata, []); title(sprintf('filtered %d', i));
            pause;
        end
        %}
        ...
        for i=1:numel(objects)
            badobject = false;
            for j=1:numel(objects(i).frame)-1
                if objects(i).frame(j)+1 ~= objects(i).frame(j+1)
                    badobject = true;
                    break;
                end
            end
            if badobject
                newframes = min(objects(i).frame):max(objects(i).frame);
            end
        end
        %}
