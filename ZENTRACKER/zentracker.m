%DONE:
%improved: displacement window is now specified in seconds instead of frames
%improved: reversal detection maximal extrapolation time is now specified in seconds instead of frames
%improved: maximal reversal duration is now specified in seconds instead of frames
%improved: minimal omega duration is now specified in seconds instead of frames
%improved: omega interruption tolerance is now specified in seconds instead of frames
%added: a warning dialogue when loading analysis data from a version earlier than zentracker 2.7.3 that results in nonstandard advanced behaviour detection parameters after converting from frames to seconds
%added: a warning dialogue when trying to auto-detect behaviour without first excluding invalid objects
%fixed: when analysis data cannot be loaded for a particular parameter from an earlier version savefile, do not try to convert the current parameter value from frames to seconds
%changed: moved the load and save analysis buttons to the botton to emphasise that the movie must be read first before the corresponding data can be loaded
%improved: made sure that revdisplacementwindowframes is never less than 1
%TODO:
...
function zentracker
    ...
    CONST_DISPLAY_COMPACTNESS = 10;
    CONST_DISPLAY_DIRECTIONCHANGE = 11;
    CONST_DISPLAY_REVERSAL = 12;
    CONST_DISPLAY_OMEGA = 13;
    CONST_DISPLAY_NNUMBER = 14;
    ...
    thresholdsizemin = 15000; %um^2 . Contiguous above-threshold areas smaller than this should not be classified as objects
    ...
    handles.scalingfactor = uicontrol(handles.scalepanel, 'Style', 'Edit', 'String', num2str(scalingfactor), 'Position', [0.05 0.05 0.40 0.65], 'Callback', {@setvalue, 'min', realmin, 'default', 1, 'setglobal', 'scalingfactor'});
    ...
    handles.timenorm = uicontrol(handles.detectionpanel, 'Style', 'Checkbox', 'String', 'Time norm', 'Position', [0.50 0.15 0.50 0.07], 'Value', timenorm, 'Callback', @settimenorm);%{@setvalue, 'logical', 'default', false, 'setglobal', 'framenorm', 'showit'});
    
    handles.trackobjects = uicontrol(handles.detectionpanel, 'Style', 'Pushbutton', 'String', 'Track objects', 'Position', [0.00 0.00 1.00 0.15], 'Callback', @trackobjects);
    ...
    handles.plotwhat = uicontrol(handles.resultspanel, 'Style', 'Popupmenu', 'String', {'Speed', 'X coordinates', 'Y coordinates', 'Length', 'Width', 'Area', 'Perimeter', 'Eccentricity', 'Solidity', 'Compactness', 'Direction change', 'Reversals', 'Omega turns', 'Valid N-numbers'}, 'Position', [0.05 0.15 0.90 0.10], 'Value', 1);
    ...
    handles.setlightarea = uicontrol(handles.fig, 'Style', 'Pushbutton', 'String', 'Set flash area', 'Position', [0.82 0.055 0.06 0.035], 'Callback', @setflasharea);
    ...
    trytoload = {'saveversion', 'objects', 'timefrom', 'timeuntil', 'scalingfactor', 'movingaverage', 'speedsmoothing', 'meanperimeter',...
            ...
            'flashed', 'flashindices', 'flashx', 'flashy', 'gradnorm', 'timenorm', 'gradnormmatrix'};
    ...
    trytosetvalue = {'identitydisplay', 'detectionareadisplay', 'thresholdeddisplay', 'measurementareadisplay', 'wormshowframeslider', 'allobjects', ...
        ...
        'detectomegas', 'gradnorm', 'timenorm'};
                        ...
                        switch whichparameter
                            ...
                            case CONST_DISPLAY_REVERSAL
                                ...
                                for j=firstindex:lastindex
                                    ...
                                    elseif objects(sortindex(i)).behaviour(j) ~= CONST_BEHAVIOUR_INVALID
            ...
            if ~(ispc && strcmpi(currentfullfile(end-3:end), '.mov')) %reading of quicktime movies is not supported on Windows, and in my experience can sometimes cause the wrong frames to be displayed (when not all frames are cached, which we cannot ensure), so we'll fall back to other options, probably mmread
                ...
                if ~successfullyread
                    try
                        ...
                        moviereaderobjects(end+1).reader = VideoReader(currentfullfile); %#ok<AGROW,TNMLP>
    ...
    function updatefilelist(hobj,eventdata) %#ok<INUSD>
        ...
        moviefileextensions = {'mpg', 'avi', 'wmv', 'asf', 'mpeg', 'mp4', 'mov', 'm4v', 'tif', 'tiff', 'czi'};
...
%                    thresholdedimage = bwlabel(thresholdedimage);
    ...
    function setgradnorm(hobj, eventdata) %#ok<INUSD>
        ...
        if gradnorm
            ...
            averageintensity = mean(toaverage, 3);
    ...
    function settimenorm(hobj, eventdata) %#ok<INUSD>
        ...
        if timenorm
            ...
            if ~any(detectionarea(:))
                if strcmpi(questdlg('It is recommended to perform across-frame intensity-normalization based on the intensities within the thresholding area, which is not currently specified.', 'Warning: thresholding area is not set up','Proceed anyway (using the entire frames)', 'Cancel and fix it','Cancel and fix it'), 'Cancel and fix it')
                    timenorm = false;
                    set(handles.timenorm, 'Value', timenorm);
                    wormshow;
                    return;
            ...
            waithandle = waitbar(0,'Processing...','Name','Calculating mean intensity drift across time', 'CreateCancelBtn', 'delete(findall(0, ''Name'',''Calculating mean intensity drift across time''))');
            
            timenormvector = NaN(1, lastframe);
            ...
            for i=1:lastframe
                ...
                if basedonmiddle
                    currentmiddle = currentframe(detectionarea);
                else
                    currentmiddle = currentframe;
                end
                timenormmeans(i) = mean(currentmiddle(:));
                timenormstds(i) = std(double(currentmiddle(:)));
            ...
            timenormvector = timenormmeans ./ mean(timenormmeans);
